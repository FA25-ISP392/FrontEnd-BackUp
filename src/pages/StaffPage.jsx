...
export default function StaffPage() {
  ...
  const [tables] = useState([
    { id: 1, number: 1, status: 'occupied', guests: 4, callStaff: false, orderTime: '14:30', duration: '45 min', totalAmount: 89.50 },
    { id: 2, number: 2, status: 'available', guests: 0, callStaff: false, orderTime: null, duration: null, totalAmount: 0 },
    { id: 3, number: 3, status: 'occupied', guests: 2, callStaff: true, orderTime: '15:00', duration: '30 min', totalAmount: 45.20 },
    { id: 4, number: 4, status: 'reserved', guests: 6, callStaff: false, orderTime: '15:30', duration: '15 min', totalAmount: 0 },
    { id: 5, number: 5, status: 'occupied', guests: 3, callStaff: false, orderTime: '14:45', duration: '40 min', totalAmount: 67.80 },
    { id: 6, number: 6, status: 'available', guests: 0, callStaff: false, orderTime: null, duration: null, totalAmount: 0 },
    { id: 7, number: 7, status: 'occupied', guests: 5, callStaff: true, orderTime: '15:15', duration: '25 min', totalAmount: 125.40 },
    { id: 8, number: 8, status: 'occupied', guests: 2, callStaff: false, orderTime: '14:50', duration: '35 min', totalAmount: 52.30 },
    { id: 9, number: 9, status: 'cleaning', guests: 0, callStaff: false, orderTime: null, duration: '5 min', totalAmount: 0 },
    { id: 10, number: 10, status: 'available', guests: 0, callStaff: false, orderTime: null, duration: null, totalAmount: 0 },
    { id: 11, number: 11, status: 'occupied', guests: 4, callStaff: false, orderTime: '15:20', duration: '20 min', totalAmount: 78.90 },
    { id: 12, number: 12, status: 'occupied', guests: 2, callStaff: false, orderTime: '14:35', duration: '50 min', totalAmount: 34.60 }
  ]);
  const [orders] = useState([
    { id: 1, table: 1, dishes: ['Caesar Salad', 'Grilled Chicken', 'Tiramisu'], status: 'preparing', total: 89.50, orderTime: '14:30', estimatedTime: '10 min' },
    { id: 2, table: 3, dishes: ['Pasta Carbonara', 'Garlic Bread'], status: 'ready', total: 45.20, orderTime: '15:00', estimatedTime: 'Ready' },
    { id: 3, table: 5, dishes: ['Margherita Pizza', 'Fish Tacos', 'Caesar Salad'], status: 'served', total: 67.80, orderTime: '14:45', estimatedTime: 'Served' },
    { id: 4, table: 7, dishes: ['Beef Burger', 'Caesar Salad', 'Chocolate Cake', 'Fries'], status: 'preparing', total: 125.40, orderTime: '15:15', estimatedTime: '15 min' },
    { id: 5, table: 8, dishes: ['Pizza Margherita', 'Coca Cola'], status: 'served', total: 52.30, orderTime: '14:50', estimatedTime: 'Served' },
    { id: 6, table: 11, dishes: ['Grilled Salmon', 'Rice', 'Vegetables'], status: 'preparing', total: 78.90, orderTime: '15:20', estimatedTime: '8 min' },
    { id: 7, table: 12, dishes: ['Pasta Alfredo'], status: 'served', total: 34.60, orderTime: '14:35', estimatedTime: 'Served' }
  ]);
          ...
          {activeSection === 'dashboard' && (
            <div className="space-y-6">
                ...
                <div className="bg-white p-6 rounded-lg shadow-sm border-l-4 border-red-500">
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-2xl font-bold text-red-600">{callStaffCount}</p>
                    </div>
              ...
              <div className="bg-white rounded-lg shadow-sm p-6">
                <h3 className="text-lg font-semibold mb-4">Trạng Thái Các Bàn</h3>
                <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
                  {tables.map((table) => (
                    <div 
                      key={table.id} 
                      className={`relative p-4 rounded-lg cursor-pointer transition-all duration-300 hover:shadow-md ${
                        table.callStaff ? 'ring-2 ring-red-500 bg-red-50' : 'bg-white border'
                      }`}
                      onClick={() => setSelectedTable(table)}
                    >
                      <div className="text-center">
                        <div className={`w-12 h-12 mx-auto mb-2 rounded-full flex items-center justify-center text-white font-bold text-lg ${getTableStatusColor(table.status)}`}>
                          {table.number}
                        </div>
                        <p className="text-sm font-medium">{getTableStatusText(table.status)}</p>
                        {table.guests > 0 && (
                          <p className="text-xs text-neutral-600">{table.guests} khách</p>
                        )}
                        {table.duration && (
                          <p className="text-xs text-neutral-500">{table.duration}</p>
                        )}
                        {table.callStaff && (
              ...
              <div className="bg-white rounded-lg shadow-sm p-6">
                ...
                <div className="space-y-3">
                  {orders.slice(0, 5).map((order) => (
                    <div key={order.id} className="flex items-center justify-between p-3 border rounded-lg hover:bg-neutral-50 transition">
                      <div className="flex items-center gap-3">
                        <div className="w-10 h-10 bg-blue-100 rounded-full flex items-center justify-center">
                          <span className="font-semibold text-blue-600">{order.table}</span>
                        </div>
                        <div>
                          <p className="font-medium">Bàn {order.table}</p>
                          <p className="text-sm text-neutral-600">{order.dishes.join(', ')}</p>
                        </div>
                      </div>
                      <div className="flex items-center gap-3">
                        ...
                        <button 
                          onClick={() => setSelectedTable(tables.find(t => t.number === order.table))}
          ...
          {activeSection === 'tables' && (
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                {tables.map((table) => (
                  <div key={table.id} className={`bg-white rounded-lg p-4 shadow-sm border-2 ${
                    table.callStaff ? 'border-red-500 bg-red-50' :
              ...
              <div className="bg-white rounded-lg p-6 shadow-sm">
                ...
                <div className="space-y-3">
                  {tables.filter(table => table.callStaff).map((table) => (
                    <div key={table.id} className="flex items-center justify-between p-3 bg-red-50 border border-red-200 rounded-lg">
                      <div>
                        <p className="font-medium">Table {table.number}</p>
                        <p className="text-sm text-neutral-600">{table.guests} guests</p>
                      </div>
                  ...
                  {tables.filter(table => table.callStaff).length === 0 && (
                    <p className="text-neutral-600 text-center py-4">No staff calls at the moment</p>
                  )}
                </div>
              </div>
            </div>
                ...
                {orders.map((order) => (
                  <div key={order.id} className="bg-white rounded-lg p-6 shadow-sm border">
                    <div className="flex items-center justify-between mb-4">
                      <div className="flex items-center gap-3">
                        <Users className="h-5 w-5 text-neutral-600" />
                        <span className="font-semibold text-lg">Table {order.table}</span>
                    ...
                    <div className="space-y-2 mb-4">
                      <h4 className="font-medium text-sm text-neutral-600">Ordered Dishes:</h4>
                    ...
                    <div className="flex gap-2">
                      ...
                      {order.status === 'served' && (
                        <span className="text-green-600 text-sm font-medium">Order Served</span>
                      )}
                    </div>
                  </div>
                ))}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50">
          <div className="bg-white rounded-lg max-w-2xl w-full max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between p-4 border-b">
              <h2 className="text-lg font-semibold">Chi Tiết Bàn {selectedTable.number}</h2>
              <button
            ...
            <div className="p-4">
              ...
              <div className="mb-6">
                <div className="flex items-center gap-4 mb-4">
                  <div className={`w-16 h-16 rounded-full flex items-center justify-center text-white font-bold text-xl ${getTableStatusColor(selectedTable.status)}`}>
                    {selectedTable.number}
                  </div>
                  <div>
                    <h3 className="text-xl font-semibold">Bàn {selectedTable.number}</h3>
                    <p className="text-neutral-600">{getTableStatusText(selectedTable.status)}</p>
                    {selectedTable.guests > 0 && (
                      <p className="text-sm text-neutral-500">{selectedTable.guests} khách</p>
                    )}
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="bg-neutral-50 p-3 rounded-lg">
                    <p className="text-sm text-neutral-600">Thời gian đặt bàn</p>
                    <p className="font-semibold">{selectedTable.orderTime || 'Chưa có'}</p>
                  </div>
                  <div className="bg-neutral-50 p-3 rounded-lg">
                    <p className="text-sm text-neutral-600">Thời gian phục vụ</p>
                    <p className="font-semibold">{selectedTable.duration || 'N/A'}</p>
                  </div>
                  <div className="bg-neutral-50 p-3 rounded-lg">
                    <p className="text-sm text-neutral-600">Tổng tiền</p>
                    <p className="font-semibold text-green-600">${selectedTable.totalAmount.toFixed(2)}</p>
                  </div>
                  <div className="bg-neutral-50 p-3 rounded-lg">
                    <p className="text-sm text-neutral-600">Trạng thái</p>
                    <p className="font-semibold">{getTableStatusText(selectedTable.status)}</p>
                  </div>
                </div>
              </div>
              ...
              <div>
                ...
                {orders.filter(order => order.table === selectedTable.number).length > 0 ? (
                  <div className="space-y-3">
                    {orders.filter(order => order.table === selectedTable.number).map((order) => (
                      <div key={order.id} className="border rounded-lg p-4">
                        ...
                        <div className="space-y-2">
                          <p className="text-sm text-neutral-600">Món đã đặt:</p>
                          <div className="space-y-1">
                            {order.dishes.map((dish, index) => (
                              <div key={index} className="flex items-center gap-2 text-sm">
                                <span className="w-2 h-2 bg-blue-600 rounded-full"></span>
          ...
          <div className="relative ml-auto w-full max-w-md bg-white shadow-xl">
            ...
            <div className="p-4">
              <div className="text-center mb-6">
                ...
                <p className="text-neutral-500 text-xs mt-1">Employee ID: STF001</p>
              </div>
