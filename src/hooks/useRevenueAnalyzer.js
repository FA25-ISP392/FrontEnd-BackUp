...
export function useRevenueAnalyzer(revenueData = [], dishSalesData = []) {
  ...
  const calculateGrowthRate = useMemo(() => {
    const currentPeriodRevenue = getFilteredRevenueData.reduce(
  ...
  const analyzeTrends = useMemo(() => {
    ...
    for (let i = 6; i < sortedData.length; i++) {
      const avg =
        sortedData
          .slice(i - 6, i + 1)
          .reduce((sum, item) => sum + item.revenue, 0) / 7;
    ...
    const sumXY = movingAverages.reduce(
      (sum, item, index) => sum + index * item.value,
      0
    );
    const sumX2 = movingAverages.reduce(
  ...
  const predictRevenue = useMemo(() => {
    ...
    const currentRevenue = getFilteredRevenueData.reduce(
      (sum, item) => sum + item.revenue,
      0
    );
    const daysInPeriod =
      revenuePeriod === "day" ? 1 : revenuePeriod === "week" ? 7 : 30;

    // Dự đoán cho kỳ tiếp theo
    const predictedRevenue =
      currentRevenue + slope * daysInPeriod * currentRevenue;
    ...
    return dishSalesData
      .map((dish) => {
        ...
        return {
          ...
          popularity:
            (totalSold / Math.max(...dishSalesData.map((d) => d.sales || 0))) *
            100,
        };
      })
