...
export function useQuantityManager(dishes = [], onSubmitRequest) {
  ...
  const batchSubmitRequests = async (dishIds) => {
    const validRequests = dishIds.filter(dishId => 
      quantities[dishId] > 0 && dishes.find(d => d.id === dishId)
    );
  ...
  const requestAnalysis = useMemo(() => {
    ...
    const rejectedRequests = requestHistory.filter(req => req.status === 'rejected').length;
    
    const totalQuantityRequested = requestHistory.reduce((sum, req) => sum + req.requestedQuantity, 0);
    const totalEstimatedCost = requestHistory.reduce((sum, req) => sum + (req.estimatedCost || 0), 0);
    ...
    return {
      ...
      averageQuantityPerRequest: totalRequests > 0 ? Math.round(totalQuantityRequested / totalRequests) : 0
    };
  }, [requestHistory]);
  ...
  const getSuggestedQuantity = (dishId) => {
    ...
    const avgQuantity = recentRequests.reduce((sum, req) => sum + req.requestedQuantity, 0) / recentRequests.length;
  ...
  const optimizeRequests = (budget = 1000) => {
    ...
    const dishProfitability = dishes.map(dish => {
