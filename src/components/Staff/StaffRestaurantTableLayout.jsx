...
export default function StaffRestaurantTableLayout({ tables, onTableClick, selectedTable }) {
  ...
  const getTableStyle = (table) => {
    let baseStyle = "w-20 h-20 border-3 rounded-xl flex items-center justify-center text-sm font-bold shadow-lg hover:shadow-xl transition-all duration-300 cursor-pointer transform hover:scale-105";
  ...
  return (
    <div className="mb-6">
      <h3 className="text-xl font-bold text-gray-800 mb-6 text-center bg-gradient-to-r from-orange-600 to-orange-800 bg-clip-text text-transparent">
        Sơ đồ bàn nhà hàng
      </h3>
      <div className="relative bg-gradient-to-br from-orange-50 via-orange-100 to-amber-50 rounded-2xl p-8 border-2 border-orange-200 shadow-xl h-96">
        ...
        <div className="absolute top-16 left-1/2 transform -translate-x-24">
          <div 
            className={getTableStyle(tables.find(t => t.number === 1) || { status: "available" })}
            title={`Bàn 1 - ${getTableStatusText(tables.find(t => t.number === 1)?.status || "available")}`}
        ...
        <div className="absolute top-16 left-1/2 transform translate-x-4">
          <div 
            className={getTableStyle(tables.find(t => t.number === 2) || { status: "available" })}
            title={`Bàn 2 - ${getTableStatusText(tables.find(t => t.number === 2)?.status || "available")}`}
        ...
        <div className="absolute bottom-16 left-1/2 transform -translate-x-24">
          <div 
            className={getTableStyle(tables.find(t => t.number === 3) || { status: "available" })}
            title={`Bàn 3 - ${getTableStatusText(tables.find(t => t.number === 3)?.status || "available")}`}
        ...
        <div className="absolute bottom-16 left-1/2 transform translate-x-4">
          <div 
            className={getTableStyle(tables.find(t => t.number === 4) || { status: "available" })}
            title={`Bàn 4 - ${getTableStatusText(tables.find(t => t.number === 4)?.status || "available")}`}
        ...
        <div className="absolute top-1/2 left-12 transform -translate-y-16">
          <div 
            className={getTableStyle(tables.find(t => t.number === 5) || { status: "available" })}
            title={`Bàn 5 - ${getTableStatusText(tables.find(t => t.number === 5)?.status || "available")}`}
        ...
        <div className="absolute top-1/2 left-12 transform translate-y-8">
          <div 
            className={getTableStyle(tables.find(t => t.number === 6) || { status: "available" })}
            title={`Bàn 6 - ${getTableStatusText(tables.find(t => t.number === 6)?.status || "available")}`}
        ...
        <div className="absolute top-1/2 right-12 transform -translate-y-16">
          <div 
            className={getTableStyle(tables.find(t => t.number === 7) || { status: "available" })}
            title={`Bàn 7 - ${getTableStatusText(tables.find(t => t.number === 7)?.status || "available")}`}
        ...
        <div className="absolute top-1/2 right-12 transform translate-y-8">
          <div 
            className={getTableStyle(tables.find(t => t.number === 8) || { status: "available" })}
            title={`Bàn 8 - ${getTableStatusText(tables.find(t => t.number === 8)?.status || "available")}`}
        ...
        <div className="absolute bottom-3 left-3 bg-white/90 backdrop-blur-sm rounded-lg p-3 shadow-lg border border-orange-200">
          <div className="text-xs font-semibold text-orange-800 mb-2">Chú thích:</div>
          <div className="space-y-1">
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs text-orange-800 font-medium">Đã đặt</span>
            </div>
            <div className="flex items-center gap-2">
              <div className="w-4 h-4 bg-gradient-to-br from-red-100 to-red-200 border-2 border-red-400 rounded-lg"></div>
              <span className="text-xs text-red-800 font-medium">Đang phục vụ</span>
            </div>
          </div>
        </div>
      </div>
