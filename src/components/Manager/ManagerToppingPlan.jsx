...
export default function ManagerToppingPlan() {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        console.error("❌ Lỗi load kế hoạch topping:", err);
      }
    })();
  }, [today]);

  const handleApproveAll = async () => {
    if (plans.length === 0) return alert("Không có kế hoạch topping để duyệt!");
    if (!window.confirm("Duyệt toàn bộ topping hôm nay?")) return;

    setLoading(true);
    try {
      await approveAllDailyPlans(plans);
      alert("✅ Đã duyệt toàn bộ topping!");
      setPlans([]);
    } catch (err) {
      console.error("❌ Lỗi duyệt topping:", err);
      alert("Không thể duyệt topping!");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleReject = async (planId, itemName) => {
    if (!window.confirm(`Từ chối yêu cầu topping "${itemName}"?`)) return;
    try {
      await deleteDailyPlan(planId);
      setPlans((prev) => prev.filter((p) => p.planId !== planId));
      alert(`❌ Đã từ chối "${itemName}"`);
    } catch (err) {
      alert("Lỗi khi xoá yêu cầu topping!");
    }
  };
  ...
  return (
    <div className="p-6">
      <div className="flex justify-between items-center mb-6">
        <h2 className="text-xl font-bold">Yêu Cầu Kế Hoạch Topping</h2>
        <button
